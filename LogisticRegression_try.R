...
g4 <- roc(CKD ~ pred4, data = testData,percent=TRUE)
plot(g4)
auc(g4)
...
#money from two models# disregard

pred2_in = plogis(predict(model2_in, newdata=data_in)) #glm predicts log odds, use plogis to get prob
pred4_in=plogis(predict(model4_in, newdata=data_in))
